...
# RESULTS_DIR = Path("/media/matt_sent/vault/dishlicker_data/data/results")  # TODO: Integrate for post-race validation
...
class SmartBettingRecommender:
    def __init__(self, date: str):
        ...
        # Define Sweet Spots (from edge finder analysis - updated 2026-02-08)
        self.sweet_spots = [
            # TOP TIER - Priority 1 (Highest ROI: 96-202%)
            SweetSpot("Q-STRAIGHT", 350, "6", 1, 30, 0.367, 225.5),
            SweetSpot("MAITLAND", 450, "4/5", 2, 10, 0.800, 139.1),
            SweetSpot("MURRAY-BRIDGE", 455, "SE", 3, 6, 0.667, 137.5),
            SweetSpot("GOULBURN", 350, "4/5", 4, 8, 0.750, 125.6),
            SweetSpot("BROKEN-HILL", 375, "MX", 5, 6, 0.667, 96.7),
            
            # HIGH TIER - Priority 2 (Good ROI: 61-85%)
            SweetSpot("WENTWORTH-PARK", 520, "M2/M3", 6, 7, 0.571, 81.4),
            SweetSpot("DUBBO", 400, "M", 7, 6, 0.667, 75.0),
            SweetSpot("Q1-LAKESIDE", 457, "M/6", 8, 5, 0.800, 74.0),
            SweetSpot("Q1-LAKESIDE", 390, "FFA", 9, 11, 0.636, 72.3),
            SweetSpot("GRAFTON", 350, "NG1-4", 10, 12, 0.667, 69.2),
            SweetSpot("ROCKHAMPTON", 407, "P5", 11, 5, 0.600, 67.0),
            SweetSpot("MANDURAH", 300, "FFA", 12, 5, 0.600, 66.0),
            SweetSpot("TAREE", 300, "M", 13, 51, 0.314, 64.9),
            SweetSpot("MAITLAND", 450, "NG", 14, 6, 0.667, 60.8),
            SweetSpot("GOSFORD", 388, "M2/M3", 15, 6, 0.667, 60.0),
        ]
    ...
    def send_no_bets_telegram(self):
        """Send Telegram message when no betting opportunities found."""
        bot_token = os.getenv('TELEGRAM_BOT_TOKEN')
        chat_id = os.getenv('TELEGRAM_CHAT_ID')
        if not bot_token or not chat_id:
            return
        try:
            ...
            data = {
                'chat_id': chat_id,
                'text': f"Model predictions ran for {self.date}, but no smart bets were identified today."
            }
            response = requests.post(url, json=data)
            if response.status_code == 200 and response.json().get('ok'):
                print("📱 Telegram sent: no bets found today")
            else:
...
def main():
    ...
    for date in dates_to_analyze:
        ...
        print("  • Box bias filtering (positive bias required for good bets)")
        
        recommender = SmartBettingRecommender(date)
        sweet_spots, good_bets = recommender.generate_recommendations()
        recommender.print_report(sweet_spots, good_bets)
        
        if sweet_spots or good_bets:
            recommender.save_betting_slip(sweet_spots, good_bets)
        
        # Add separator if analyzing multiple dates
        if len(dates_to_analyze) > 1 and date != dates_to_analyze[-1]:
            print("\n" + "="*80)
            print()  # Extra line break between dates
